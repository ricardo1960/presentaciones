...
\end{exampleblock}
\onslide*<2-2>{
Vamos a calcular los dos puntos más cercanos entre sí de las dos rectas y con estos dos puntos calcularemos la distancia mínima y la perpendicular común.
}
\onslide*<3->{
Ponemos la recta $r$ en forma parámetrica: \pause $r \equiv \begin{cases} x=  \mu \\ y = \mu \\ z= \mu \end{cases}$}

\onslide*<4->{
Tomamos un punto genérico de cada recta y hallamos el vector que une dichos puntos:
}
\onslide*<5->{
\[ \left. \begin{array}{l} R(\mu,\mu,\mu) \\ S(1 + \lambda, 3+\lambda, -\lambda) \end{array} \right\rbrace \Rightarrow \vector{RS}=(1 + \lambda-\mu, 3+\lambda-\mu, -\lambda-\mu) \]
}
\end{frame}
...
Imponemos la condición de que este vector sea perpendicular a cada uno de los vectores directores de la recta y de esta forma calcularemos los dos puntos más cercanos.
\onslide*<2->{
\[ \begin{array}{l}
\vector{RS} \bot \vector{v_r} \Rightarrow \vector{RS} \cdot \vector{v_r}=0 \Rightarrow 1 + \lambda-\mu + 3+\lambda-\mu -\lambda-\mu=0  \\
\vector{RS} \bot \vector{v_s} \Rightarrow \vector{RS} \cdot \vector{v_s}=0 \Rightarrow 1 + \lambda-\mu + 3+\lambda-\mu +\lambda+\mu=0 
\end{array} \]}
\onslide*<3->{
Resolvemos el sistema y hallamos $\mu$ y $\lambda$.
}
\onslide*<4->{
\[ \left. \begin{array}{l} \lambda -3\mu = -4 \\ 3\lambda - \mu = -4 \end{array} \right\rbrace \Rightarrow \begin{cases} \mu = 1 \\ \lambda=-1 \end{cases} \]
}
\onslide*<5->{
Con estos valores hallamos los dos puntos y el vector que los une:
}
\onslide*<6->{
\[ R(1,1,1) \qquad S(0,2,1) \qquad \vector{RS}= (-1,1,0) \]
}
...
Con estos dos puntos calcularemos la distancia entre las dos rectas y la perpendicular común.

\onslide*<2->{
La distancia entre las dos rectas será la distancia entre estos dos puntos.
}
\onslide*<3->{
\[ d(r,s)=d(R,S)=\sqrt{(0-1)^2+(2-1)^2+(1-1)^2} \]
\resultado{$d(r,s)= \sqrt{2} \, \text{u}$}
}

\onslide*<4->{
La perpendicular común es la recta que pasa por estos dos puntos. 

Para calcularla utilizaremos el punto $R$ y el vector $\vector{RS}$
}

\onslide*<5->{
\resultado{$ t \equiv \begin{cases} x= 1-\lambda \\ y= 1 +\lambda \\ z= 1 \end{cases}$}
}
\end{frame}
...
\end{exampleblock}
\onslide*<2->{
Primero examinamos la posición relativa de la recta y el plano.
}

\onslide*<3->{
\[ \begin{array}{|l|l|} \hline r \equiv \begin{cases} x= \lambda \\ y = -3-\lambda \\ z= 2\lambda \end{cases} \Rightarrow \vec{v}=(1,-1,2)  & \pi \equiv 3x+y-z+4=0 \Rightarrow \vec{n}=(3,1,-1) \\ \hline \end{array}\]
}
\onslide*<4->{
\[ \left. \begin{array}{l} \vec{v}\cdot \vec{n}= 3-1-2 = 0 \\ 3\cdot 0 +(-3)- 0 +4= 1 \neq 0 \end{array} \right\rbrace \Rightarrow \text{Paralelos} \]
}

\onslide*<5->{
\[ d(r,\pi)=\dfrac{|3\cdot 0 +(-3)- 0 +4|}{\sqrt{3^2+1^2+(-1)^2}}=\dfrac{1}{\sqrt{11}} \]

\resultado{$\dfrac{\sqrt{11}}{11} \text{u}$}
}
\end{frame}
